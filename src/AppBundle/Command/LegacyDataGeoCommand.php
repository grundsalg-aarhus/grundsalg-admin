...
//              try {
//                $sql = 'UPDATE Grund SET geo = ST_GEOMFROMTEXT(?, ?) WHERE id = ?';
//
//                $stmt = $connection->prepare($sql);
//                $stmt->bindValue(1, $row['WKT']);
//                $stmt->bindValue(2, $row['srid']);
...
//    $this->printChangeLog($this->changeForeignKeyCount);
  ...
  private function matchToOmraade($table, $adresse, $pris, $grundPris, $m2, $omraadeId)
  {
    ...
    try {
      ...
      $stmt = $connection->prepare($sql);
      $stmt->bindValue(1, $m2);
      $stmt->bindValue(2, $pris);
      $stmt->bindValue(3, $vej);
      $stmt->execute();
    ...
    $c = count($result);
    $this->printWarning('Unknown Grund: ' . $table . ' / ' . $adresse . ', m2: ' . $m2 . ', pris: ' . $pris . ', gPris: ' . $grundPris . ' matches ' . $c . ' rows');

  }

  private function matchToGrund($table, $adresse, $pris, $grundPris, $m2, $omraadeId)
  {
    $connection = $this->getContainer()->get('doctrine.dbal.default_connection');
